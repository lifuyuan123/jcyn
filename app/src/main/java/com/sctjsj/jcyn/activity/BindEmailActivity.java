...
public class BindEmailActivity extends BaseAppcompatActivity {
...
//        getWindow().setStatusBarColor(getResources().getColor(R.color.app_white));
   ...
   public void bindOnclick(View view){
       switch (view.getId()){
           ...
           case R.id.bind_new_bt:
               if(editText.getText().length()>0 && bindEt.getText().length()>0){
                   ...
                   Toast.makeText(BindEmailActivity.this,"请输入完整信息",Toast.LENGTH_LONG).show();
               }
               break;
           case R.id.send_noction_bt:
               if(editText.getText().length()>0){
                   if(RegexpValidateUtil.checkEmail(editText.getText().toString())){
                       ...
                       Toast.makeText(BindEmailActivity.this,"请输入正确邮箱",Toast.LENGTH_LONG).show();
                   }
               }else {
                   Toast.makeText(BindEmailActivity.this,"请输入邮箱",Toast.LENGTH_LONG).show();
               }
               break;
       }
    ...
    public void sendEmail(){
        ...
        params.addBodyParameter("isbind","1");
    ...
    public void bindEmail(){
        ...
        x.http().post(params, new Callback.CommonCallback<JSONObject>() {
            ...
            public void onSuccess(JSONObject result) {
                ...
                try {
                    if(result.getBoolean("result")){
                        Toast.makeText(BindEmailActivity.this,"绑定成功",Toast.LENGTH_LONG).show();
                        Intent i = new Intent(BindEmailActivity.this,SetActivity.class);
                        startActivity(i);
                        finish();
                    }else {
                        Toast.makeText(BindEmailActivity.this,"绑定失败",Toast.LENGTH_LONG).show();
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
